...
//     node* initialNodes[10]; // Supports multiple initial states
...
//             char *start = strstr(line, "doublecircle]") + 12;  // Move past "doublecircle]"
...
//            char from[100] = {0}, to[100] = {0}, label[100] = {0};

//             sscanf(line, " %[^->] -> %[^ [] [label=\"%[^\"]\"];", from, to, label);
